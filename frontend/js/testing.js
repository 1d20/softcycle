(function(window) {
    ...
    GameState.prototype.preload = function() {
        this.game.load.image('cloud', '/static/frontend/images/games/tests/ball.png');
        this.game.load.image('ground', '/static/frontend/images/games/tests/ground.png');
        this.game.load.spritesheet('bug', '/static/frontend/images/games/tests/monster.png', 32, 32);
        this.game.load.spritesheet('monster', '/static/frontend/images/games/tests/monster.png', 32, 32);
...
GameState.prototype.create = function() {
    ...
    this.bugGroup.createMultiple(BAGS, 'bug', 1);
    ...
    this.highscoreText = this.game.add.text(
        this.game.width - 240, 20, '', { font: '16px Arial', fill: '#ffffff' }
    );
};
...
GameState.prototype.update = function() {
    ...
    var codestat = Config.pathed.code - Config.killed.code;
    var bugstat = Config.killed.bugs - Config.pathed.bugs;
    this.highscoreText.setText("Highscore: bugs: " + bugstat + ' code: ' + codestat);
    // Spawn a new monster
    ...
    this.bugGroup.forEachAlive(function(bug) {
        if (bug.x < -64 || bug.y > this.game.height) {
            //Config.pathed.bug++;
            bug.kill();
        }
    }, this);
    ...
    if (this.game.input.activePointer.justPressed(20)) {
        ...
        this.bugGroup.forEachAlive(function(bug) {
            if (this.game.math.distance(
                    this.game.input.activePointer.x, this.game.input.activePointer.y,
                    bug.x, bug.y) < 64) {
                bug.frame = 1; // Show the "dead" texture
